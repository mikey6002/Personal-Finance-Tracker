...
class Start_Screen(MDApp):
        ...
        for kv in kv_files:
            if os.path.exists(kv):
                ...
                if isinstance(loaded_widget, Screen):
                    ...
                    print(f"Loaded screen: {loaded_widget.name}")

        print(f"Available screens: {screen_manager.screen_names}")
        return screen_manager

    
    def show_transactions_screen(self):
        # This method will be responsible for displaying the transactions screen
        self.root.current = "transactions"
        self.load_transactions()

    def load_transactions(self):
        csv_file_path = 'user_data.csv'  # Adjust the path as needed
        ...
        self.root.get_screen('transactions').ids.data_table_box.add_widget(self.data_table)
    ...
    def categories_dropper(self):
        ...
        self.menu = MDDropdownMenu(
            caller=self.root.get_screen('transactions').ids.categories,
            items=menu_items,
            width_mult=3
        )
        self.menu.open()
    ...
    def menu_callback(self, widget_id, text_item):
        self.root.get_screen('transactions').ids[widget_id].text = text_item
        self.menu.dismiss()
